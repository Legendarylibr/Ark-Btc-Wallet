...
- `POST /api/auth/barkd-ready` requires **unlock-check token** + pre-session Ed25519; before pairing returns **daemon reachability only**; after pubkey pin requires **matching signing key** + wallet file check; public `GET /api/wallet/ready` is deprecated (410)
